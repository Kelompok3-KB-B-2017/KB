...
def fix_atari(pos, c, singlept_ok=False, twolib_test=True, twolib_edgeonly=False):
    ...
    while True:
        ...
        boardcc = board_put(boardcc, othergroup, '%')
    ...
    fboard = floodfill(posesc.board, l)
    ...
    if l_new_2 is not None:
        ...
        if solutions or not (contact(board_put(fboard, l_new_2, 'L'), '.') is None
                             and read_ladder_attack(posesc, l, l_new, l_new_2) is not None):
            solutions.append(l)
...
def cfg_distances(board, c):
    ...
    while fringe:
        ...
        for d in neighbors(c):
            ...
            if board[d] == board[c] and board[d] != '.':
                cfg_map[d] = cfg_map[c]
            else:
...
def inout_game(computer_black=False):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        inout_game(computer_black=True)
    elif sys.argv[1] == "gtp":
        gtp_io()
